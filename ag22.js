...
const client2 = new DeliverooApi(
    //'http://localhost:8080', client1.token
    'http://localhost:8080/?name=SVM_2',client1.token
    //'http://localhost:8080',
    //'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpZCI6IjM1ZDg0YWMwN2RhIiwibmFtZSI6IlNWTV8yIiwiaWF0IjoxNzE5OTM0MDMwfQ.Tbr2ZjeVqBdUFaCZgxg_8t1WtNns9d7lCIb5oc_Pj_s'
...
client1.onMap( (width, height, tiles) => {
    map.width = width;
    map.height = height;
    ...
    pathPlanning2 = new PathPlanning();

} )

client1.onTile( (x, y, delivery) => {
    map.add( {x, y, delivery} );
} )

client1.onNotTile( ( x, y ) => { 
...
client1.onAgentsSensing( ( sensed_agents ) => {
...
client1.onParcelsSensing( async ( perceived_parcels ) => {
...
var reset1 = true;
client1.onParcelsSensing( parcels => {
    ...
    for (const parcel of parcels.values()) {
        ...
        if ( ! parcel.carriedBy) {
            options.push( [ 'go_pick_up', parcel.x, parcel.y, client1 ]); //parcel.id
        }
    }
    ...
    if ( best_option ){
        ...
        if (me.carrying.size > 0){
            myAgent.push( ['deliv1', nearestDelivery(me.x,me.y).x, nearestDelivery(me.x,me.y).y, client1])
        }
...
class GoPickUp extends Plan {
    ...
    async execute ( go_pick_up, x, y, client ) {
        if ( this.stopped ) throw ['stopped'];
        await this.subIntention( ['go_to', x, y, client1] );
        if ( this.stopped ) throw ['stopped'];
        await client.pickup()
        if ( this.stopped ) throw ['stopped'];
        return true;
...
class GoDeliver extends Plan {
    ...
    static isApplicableTo ( go_deliver, x, y,) {
        return go_deliver == 'deliv1';
    }
    async execute ( go_deliver, x, y, client ) {
...
class BlindMove extends Plan {
    ...
    async execute ( go_to, x, y, client ) {
...
class ExplRandom extends Plan {
    ...
    async execute ( go_pick_up, x, y ) {
        ...
        if ( this.stopped ) throw ['stopped'];
        await this.subIntention( ['go_to', x_r, y_r, client1] );
        if ( this.stopped ) throw ['stopped'];
        await client.pickup()
        if ( this.stopped ) throw ['stopped'];
        return true;
