    ...
    if (msg.hello == 'parcel_spotted') {
        ...
        if (msg.is_sec.some(sec => sec[0] === me.x && sec[1] === me.y)){
            ...
            if(isInIsolatedSection.length > 0) {
                isolated_single = 1; 
                myAgent.push( ['isolated_section_single', isInIsolatedSection]);}
            } 
            reply(false);
        }
});
...
client.onParcelsSensing( async(parcels) => {
    ...
    for (const parcel of parcels.values()) {
        ...
        if (isolated_single == 0) {
            const reply = await client.ask( 'bb5bb5a2b07', {
                px: parcel.x,
                py: parcel.y,
                pid: parcel.id,
        ...
        if (map2.xy(parcel.x, parcel.y) == undefined){ignored_parcels.push(parcel.id)};
    ...
    if ( best_option ){
        ...
        else if (when_idle === 4){ myAgent.push( ['block_spawn', me.x, me.y])}
        else if (when_idle === 3){ myAgent.push( ['block_deliv', me.x, me.y])}
...
class ExplRandom extends Plan {
    ...
    async execute ( expl_rand, x, y ) {
        ...
        if (map2.spwn().length < 1){return this.subIntention( ['go_to', x_r0, y_r0] );}

        // --> if there is at least one, it picks a random spawnable tile (possible to add closer spawnable tile or other conditions)
        // in the agent half of the map
        let randomIndex = Math.floor(Math.random() * map2.spwn().length);
        let selectedPoint = map2.spwn()[randomIndex];
        var x_r = selectedPoint.x;
        var y_r = selectedPoint.y;
...
// BlockDelivery and BlockSpawn are agent2 special plans
class BlockDelivery extends Plan {

    static isApplicableTo ( block_deliv, x, y, id ) {
        return block_deliv == 'block_deliv';
    }
    async execute () {
        ...
        console.log('Blocking deliv', x_r, y_r);

        if ( this.stopped ) throw ['stopped'];
        await this.subIntention( ['go_to', x_r, y_r] );
        if ( this.stopped ) throw ['stopped'];
        await new Promise(async(resolve) => setTimeout(resolve, 999999999))
        //await client.timer(1000000000000000000000000000);
        if ( this.stopped ) throw ['stopped'];
        return true;
    }
}
...
class BlockSpawn extends Plan {

    static isApplicableTo ( block_spawn, x, y, id ) {
        return block_spawn == 'block_spawn';
    }
    async execute () {
        ...
        console.log('Blocking spawn', x_r, y_r);

        if ( this.stopped ) throw ['stopped'];
        await this.subIntention( ['go_to', x_r, y_r] );
        if ( this.stopped ) throw ['stopped'];
        await new Promise(async(resolve) => setTimeout(resolve, 999999))
        if ( this.stopped ) throw ['stopped'];
        return true;
    }
}
